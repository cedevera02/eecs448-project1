...
class player
{
    ...
    void playerTurn(int x, int y, bool hitCheck, bool useMissile);//this function updates the player's shot grid
        bool shoot(int x, int y);//returns true for valid input, false otherwise
    std::string updatePlayerShotAt(int x, int y, bool useMissile);//returns "**HIT!**", "**Miss**", "Ship of size <s> destroyed!", updates the ships and ship board when the opponent fires
        bool hitCheck(int x, int y, bool useMissile);//returns true if hit, false otherwise, this is called when the opponent attacks
