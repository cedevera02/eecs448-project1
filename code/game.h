...
class game
{
    ...
    bool m_missileGame;///< bool that is true when the game enablers bissles, and false when the special game mode isn't enabled.
    ...
    void finishTurnPrompt();///< Allows the user to see the result of their turn before the screen gets cleared
    bool isStringInt(std::string s);///< checks a string to see if is an int
    bool isStringLetter(std::string s);///< checks the string to see if it's a letter
